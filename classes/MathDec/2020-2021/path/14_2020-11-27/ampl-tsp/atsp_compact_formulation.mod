/*
May 2019 - This GMPL model was created by Alice Raffaele (alice.raffaele@unitn.it) for teaching purposes.

ASYMMETRIC TRAVELING SALESMAN PROBLEM (ATSP)
Input: a directed graph G = (V,A), a weight function w: A -> R+;
Output: a Hamiltonian cycle which visits all the vertices in V exactly once.

This model implements Miller, Tucker and Zemlin's compact formulation for the ATSP,
enhanced by Sawik. For more details, see: 
...
Instead of introducing an exponential number of constraints to avoid subtours in feasible solutions,
starting from Dantzig, Fulkerson and Johnson's formulation, Miller, Tucker and Zemlin propose to give an ordering to all nodes.
...
subject to OnlyOneIncoming{j in V}: # to each vertex, only one incoming arc is selected
	sum{(i,j) in A: i <> j} x[i,j] = 1;
subject to Either{i in V, j in V: i <> j}: # the tour may contain either the arc x[i,j] or x[j,i], or none of them
	x[i,j] + x[j,i] <= 1;
