...
class Tableau:
    ...
    def __init__(self, n, m, obj, prob_type, termine_noto_c = 0):
        self.n = n
        self.m = m
        self.rows = []
        self.cons = []
        self.basis = []
        self.termine_noto_c = termine_noto_c
        if prob_type == 'max':
    ...
    def mostra_tableau(self):
        ...
        for i in range(1,len(self.obj)-1):
            s += '\t' + str(self.obj[i])
        s += '\t' + str(self.obj[-1]-termine_noto_c)
        print(s)
    ...
    def stampa_soluzione_base_corrente(self):
        ...
        for i in range(len(sol)):
            if i != len(sol)-1:
                ...
                s += str(sol[i]) + ')'
        print(s)
        print('Funzione obiettivo = ' + str(-(self.obj[-1])+termine_noto_c))
...
if __name__ == "__main__":
    ...
    termine_noto_c = 3
    b = [4,2,1]
    A = [[Fraction(1)/Fraction(3),1], [-2,1], [1,-1]]
    n = 2 # numero variabili
    m = 3 # numero vincoli
    
    t = Tableau(n,m,c,'max',termine_noto_c) # dimensioni del problema, funzione obiettivo e tipologia di problema (sono consentiti 'max' e 'min')
    # Vincoli
